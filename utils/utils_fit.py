...
def fit_one_epoch(diffusion_model_train, diffusion_model, loss_history, optimizer,
    ...
    for iteration, img in enumerate(gen):
        images = img[0]
        conditions = img[1]
        ...
        with torch.no_grad():
            if cuda:
                ...
                conditions = conditions.cuda(local_rank)
        
        if not fp16:
            optimizer.zero_grad()
            diffusion_loss = torch.mean(diffusion_model_train(images, conditions))
    ...
    if local_rank == 0:
        ...
        show_result(epoch + 1, diffusion_model, 'cuda')
